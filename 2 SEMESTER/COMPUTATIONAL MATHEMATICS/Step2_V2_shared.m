...
while zm(cc)>-3
    ...
    if zm(cc)>-2.9
     ...
     %% Posición respecto al tiempo (Figura 2)
     zm(cc+1)=zm(cc)+vz(cc)*dt+0.5*a*dt*dt;   % Fórmula UAM con efecto fuerza mag
     zmfree(cc+1)=zmfree(cc)+vzfree(cc)*dt-0.5*9.81*dt*dt;    % Sin efecto fuerza mag
    
        vz(cc+1)=(zm(cc+1)-zm(cc))/dt;    % Cambio en la posición con respecto al tiempo   
